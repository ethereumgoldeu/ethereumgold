...
#if defined(NDEBUG)
# error "EthereumGold cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xaeb8f71e278b8d69447a9363c6290ccb8870fc8ca28f351e71100eeb7bf58d51");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // EthereumGold: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "EthereumGold Signed Message:\n";
...
{
    // EthereumGold: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 140 * COIN;

            if(nHeight == 1)
            {
            nSubsidy = 1053763 * COIN;
            }
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // EthereumGold: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // EthereumGold: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1510647017, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "the times 14 November 2017";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 140 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("044601b1aafb62459f108c66b454cfbb36757583ef142197fbf2cabadad19f39d4e013d12ad41e18b02c9bfb9bde3831dbc7054d01668ac99b01504d5d5f5867c5") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1510647017;
        ...
        assert(block.hashMerkleRoot == uint256("0x87f988f2d28b6b67c5832e16251ebdcc2540000cf091c79dd88b02c04a578932"));
...
unsigned char pchMessageStart[4] = { 0x8e, 0xb0, 0xce, 0xc2 }; // EthereumGold: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EthereumGoldMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EthereumGoldMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static EthereumGoldMiner(CWallet *pwallet)
{
    printf("EthereumGoldMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ethereumgold-miner");
    ...
    try { loop {
        ...
        printf("Running EthereumGoldMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("EthereumGoldMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EthereumGoldMiner, pwallet));
}
